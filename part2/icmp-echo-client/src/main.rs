...
const CODE: [u8; CODE_LEN] = [b' ', b'D', b'1', b'6'];
const ICMP_HEADER_LEN: usize = 4;
const ECHO_HEADER_LEN: usize = 4;
const CODE_WITH_ECHO_HEADER_LEN: usize = ECHO_HEADER_LEN + CODE_LEN;
const BSD_PING_WITH_TIMESTAMP_LEN: usize = ECHO_HEADER_LEN + 8;
const GNU_PING_WITH_TIMESTAMP_LEN: usize = ECHO_HEADER_LEN + 16;
	...
	loop {
		match rx_iter.next() {
			Ok((packet, addr)) => {
				...
				let mut buf_len = ECHO_HEADER_LEN + payload_len;
				buf[ICMP_HEADER_LEN..(ICMP_HEADER_LEN + payload_len)].copy_from_slice(&payload[..payload_len]);
				match payload_len {
					0..CODE_WITH_ECHO_HEADER_LEN | BSD_PING_WITH_TIMESTAMP_LEN | GNU_PING_WITH_TIMESTAMP_LEN => {
						buf_len += CODE_LEN;
						buf[(ICMP_HEADER_LEN + payload_len)..buf_len].copy_from_slice(&CODE);
					},
					_ => {
						let offset = match payload_len {
							0..=BSD_PING_WITH_TIMESTAMP_LEN => ECHO_HEADER_LEN,
							0..=GNU_PING_WITH_TIMESTAMP_LEN => BSD_PING_WITH_TIMESTAMP_LEN,
							_ => GNU_PING_WITH_TIMESTAMP_LEN
						};
						buf_len = max(buf_len, ICMP_HEADER_LEN + offset + CODE_LEN);
						buf[(ICMP_HEADER_LEN + offset)..(ICMP_HEADER_LEN + offset + CODE_LEN)].copy_from_slice(&CODE);
					}
				};
